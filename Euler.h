...
template <class LabelType>
void Euler<LabelType>::setStartingVertex(LabelType & start, LabelType & end)
{
	DACmapIterator<LabelType, Vertex<LabelType>*> * iter = this->vertices.iterator();
	...
	while (iter->hasNext())
	{
		...
		if (loopVertex->getNumberOfNeighbors() % 2 == 1)
		{
			start = loopVertex->getLabel();
			end = loopVertex->getNextNeighbor();
			break;
		}
	}
}
